...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const deleteImageCircleBtn = document.querySelector('.delete-image-circle');
  const checkNowBtn = document.querySelector('.check-now');
      ...
      if (this.files && this.files[0]) {
          ...
          if (image.size < 2000000) {
              ...
              reader.onload = () => {
                  ...
                  deleteImageCircleBtn.style.display = 'block';
                  getBase64(image); // Convert image to base64 when a new image is loaded
              };
  ...
  deleteImageCircleBtn.addEventListener('click', function () {
      const allImg = imgArea.querySelectorAll('img');
      allImg.forEach(item => item.remove());
...
function uploadImage() {
    var file = document.getElementById('file').files[0]; // Ensure this matches your file input's ID
    getBase64(file); // Convert image to base64 string
}